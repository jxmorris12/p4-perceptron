...
class Topology:
    ...
    def add_host(self, h_id, h_ip=None, h_mac=None):
        if h_id in self.G.nodes:
            print ("%s is already in the topology as a %s." \
                    % (h_id, self.G.nodes[h_id]["typ"]))
            return

        self.G.add_node(h_id, typ = "host", ip = h_ip, mac = h_mac)
    ...
    def ip(self, h):
        return self.G.nodes[h]["ip"]

    def mac(self, h):
        return self.G.nodes[h]["mac"]
    ...
    def e2e_shortest_paths(self):
        ...
        for n1 in paths:
            ...
            for n2 in paths[n1]:
                if self.G.nodes[n2]["typ"] == "switch" or n1 == n2:
                    continue
